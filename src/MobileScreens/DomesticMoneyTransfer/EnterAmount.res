...
let make = (~handleNavigate) => {
  <div className="flex flex-col h-full justify-between">
    <img
      src="/EnterAmountTop.svg" alt="Description of image" className="scale-105 border border-white"
    />
    <div className="flex flex-col gap-1  ">
      <div className="font-medium text-[#797979] text-sm"> {React.string("Amount")} </div>
      <div className="flex flex-row border border-lightGrayBorder-rgba rounded-2xl h-14">
    ...
    <div className="flex flex-col gap-4 mt-5">
      ...
      <img src="/KeypadIphone.svg" alt="Description of image" className="scale-105" />
    </div>
  </div>
}
